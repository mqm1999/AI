...
public class YoloV5Classifier implements Classifier {
    ...
    /**
     * init TensorFlow session để nhận dạng hình ảnh
     * các tham số truyền vào
     * @param assetManager  để load các assets.
     * @param modelFilename đường dẫn của model GraphDef
     * @param labelFilename đường dẫn chứa label của các class
     * @param isQuantized   biến boolean xác định trạng thái lượng tử hóa
     */
...
//    private int[] ANCHORS;
    ...
    protected ArrayList<Recognition> nms(ArrayList<Recognition> list) {
        ...
        for (int k = 0; k < labels.size(); k++) {
            //  tìm max confident cho mỗi class
                    ...
                    new PriorityQueue<Recognition>(
                            ...
                            new Comparator<Recognition>() {
                                ...
                                public int compare(final Recognition lhs, final Recognition rhs) {
                                    // chuyển giá trị confidence cao hơn lên trước trong queue
            ...
            //  loại bỏ các bounding box thừa
            while (pq.size() > 0) {
                //insert detection với confidence lớn nhất
    ...
    protected float overlap(float x1, float w1, float x2, float w2) {
        float l1 = x1 - w1 / 2;
        float l2 = x2 - w2 / 2;
        float left = l1 > l2 ? l1 : l2;         // if l1 > l2 => left = l1, otherwise left = l2
        float r1 = x1 + w1 / 2;
        float r2 = x2 + w2 / 2;
        float right = r1 < r2 ? r1 : r2;        // if r1 < r2 => right = r1, otherwise right = r2
        return right - left;
    }
    ...
    /**
     * convert từ image về bytebuffer
     */
